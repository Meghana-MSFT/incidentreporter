...
namespace Microsoft.Teams.Apps.RemoteSupport.Helpers
{
    ...
    public class SearchHelper : ISearchHelper
    {
        ...
        public SearchHelper(ICardHelper cardHelper)
        {
            ...
            string onCallSMEUsers = "")
        {
            ...
            switch (commandId)
            {
                case Constants.UrgentCommandId:
                    ...
                    composeExtensionResult = this.GetMessagingExtensionResult(searchServiceResults, localizer, commandId);
                    break;
                ...
                case Constants.ActiveCommandId:
                    ...
                    composeExtensionResult = this.GetMessagingExtensionResult(searchServiceResults, localizer, commandId, onCallSMEUsers);
                    break;
        ...
        public MessagingExtensionResult GetMessagingExtensionResult(
            IList<TicketDetail> searchServiceResults,
            IStringLocalizer<Strings> localizer,
            string commandId = "",
            string onCallSMEUsers = "")
        {
            ...
            if (searchServiceResults != null)
            {
                foreach (var ticket in searchServiceResults)
                {
                    ...
                    {
                        this.cardHelper.GetAdaptiveCardColumnSet(localizer.GetString("RequestNumberText"), $"#{ticket.TicketId}"),
                        this.cardHelper.GetAdaptiveCardColumnSet(localizer.GetString("TitleDisplayText"), ticket.Title),
                        this.cardHelper.GetAdaptiveCardColumnSet(localizer.GetString("DescriptionText"), ticket.Description),
                        this.cardHelper.GetAdaptiveCardColumnSet(localizer.GetString("CreatedOnText"), ticket.CreatedOn.ToString(CultureInfo.InvariantCulture)),
                    };
                    ...
                    if (commandId == Constants.ActiveCommandId && !string.IsNullOrEmpty(onCallSMEUsers))
                    {
                        commandIdCard.Actions.Add(
                            new AdaptiveOpenUrlAction
                            {
                                ...
                                Url = new Uri(this.CreateGroupChat(onCallSMEUsers, ticket.TicketId, ticket.RequesterName, localizer)),
                            });
                    }
                    else if ((commandId == Constants.UrgentCommandId || commandId == Constants.AssignedCommandId || commandId == Constants.UnassignedCommandId) && ticket.SmeConversationId != null)
                    {
                        commandIdCard.Actions.Add(
                            new AdaptiveOpenUrlAction
                            {
                                ...
                                Url = new Uri(this.CreateDeepLinkToThread(ticket.SmeConversationId)),
                            });
                    }
        ...
        private string CreateDeepLinkToThread(string threadConversationId)
        {
        ...
        private string CreateGroupChat(string onCallSMENames, string ticketId, string requesterName, IStringLocalizer<Strings> localizer)
        {
