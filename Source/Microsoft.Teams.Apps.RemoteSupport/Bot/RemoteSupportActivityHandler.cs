...
namespace Microsoft.Teams.Apps.RemoteSupport
{
    ...
    public sealed class RemoteSupportActivityHandler : TeamsActivityHandler
    {
        ...
        /// <summary>
        /// Helper that handles Bot activity helper methods.
        /// </summary>
        ...
        /// <param name="cardHelper">Helper that handles the card configuration.</param>
        /// <param name="activityHelper">Helper that handles Bot activity helper methods.</param>
        ...
        /// <param name="searchHelper">Class that handles the search activities for messaging extension.</param>
            ...
            try
            {
                ...
                switch (activity.Conversation.ConversationType)
                {
                    case PersonalConversationType:
                        await this.activityHelper.OnMessageActivityInPersonalChatAsync(activity, turnContext, this.logger, this.cardConfigurationStorageProvider, this.ticketGenerateStorageProvider, this.ticketDetailStorageProvider, this.microsoftAppCredentials, this.appBaseUrl, this.localizer, cancellationToken);
                        break;

                    case ChannelConversationType:
                        await this.activityHelper.OnMessageActivityInChannelAsync(message: activity, turnContext: turnContext, onCallSupportDetailSearchService: this.onCallSupportDetailSearchService, ticketDetailStorageProvider: this.ticketDetailStorageProvider, cardConfigurationStorageProvider: this.cardConfigurationStorageProvider, logger: this.logger, appBaseUrl: this.appBaseUrl, localizer: this.localizer, cancellationToken: cancellationToken);
                        break;
            ...
            CancellationToken cancellationToken)
        {
            try
            {
                ...
                {
                    case PersonalConversationType:
                        await this.activityHelper.OnMembersAddedToPersonalChatAsync(membersAdded: activity.MembersAdded, turnContext: turnContext, logger: this.logger, appBaseUrl: this.appBaseUrl, this.localizer);
                        return;

                    case ChannelConversationType:
                        await this.activityHelper.OnMembersAddedToTeamAsync(membersAdded: activity.MembersAdded, turnContext: turnContext, microsoftAppCredentials: this.microsoftAppCredentials, logger: this.logger, appBaseUrl: this.appBaseUrl, localizer: this.localizer, cancellationToken: cancellationToken);
                        return;
            ...
            try
            {
                ...
                switch (command.ToUpperInvariant())
                {
                    case Constants.EditRequestAction:
                        ...
                        {
                            return this.cardHelper.GetClosedErrorAdaptiveCard(this.localizer);
                        }

                        this.logger.LogInformation("Fetch and send edit request card.");
                        return this.cardHelper.GetEditTicketAdaptiveCard(cardConfigurationStorageProvider: this.cardConfigurationStorageProvider, ticketDetail: ticketDetail, localizer: this.localizer);
                    ...
                    case Constants.ManageExpertsAction:
                        ...
                        return this.cardHelper.GetTaskModuleResponse(applicationBasePath: this.appBaseUrl, customAPIAuthenticationToken: customAPIAuthenticationToken, telemetryInstrumentationKey: this.telemetryClient.InstrumentationKey, activityId: valuesforTaskModule?.ActivityId, localizer: this.localizer);
                    default:
        ...
        protected override async Task<TaskModuleResponse> OnTeamsTaskModuleSubmitAsync(ITurnContext<IInvokeActivity> turnContext, TaskModuleRequest taskModuleRequest, CancellationToken cancellationToken)
        {
            ...
            switch (valuesforTaskModule.Command.ToUpperInvariant())
            {
                case Constants.UpdateRequestAction:
                    ...
                    if (this.ticketHelper.ValidateRequestDetail(editTicketDetail))
                    {
                        ticketDetail.AdditionalProperties = this.cardHelper.ValidateAdditionalTicketDetails(((JObject)activity.Value).GetValue("data", StringComparison.OrdinalIgnoreCase)?.ToString(), turnContext.Activity.LocalTimestamp.Value.Offset);

                        // Update request card with user entered values.
                        ticketDetail = this.ticketHelper.GetUpdatedTicketDetails(turnContext, ticketDetail, editTicketDetail);
                        ...
                        IMessageActivity ticketDetailActivity = MessageFactory.Attachment(this.ticketCard.GetTicketDetailsForPersonalChatCard(cardElementMapping, ticketDetail, this.localizer, true));
                        ...
                        await this.cardHelper.UpdateSMECardAsync(turnContext, ticketDetail, smeEditNotification, this.appBaseUrl, cardElementMapping, this.localizer, this.logger, cancellationToken);
                    }
                    else
                    {
                        editTicketDetail.AdditionalProperties = this.cardHelper.ValidateAdditionalTicketDetails(additionalDetails: ((JObject)activity.Value).GetValue("data", StringComparison.OrdinalIgnoreCase)?.ToString(), timeSpan: turnContext.Activity.LocalTimestamp.Value.Offset);
                        return this.cardHelper.GetEditTicketAdaptiveCard(cardConfigurationStorageProvider: this.cardConfigurationStorageProvider, ticketDetail: editTicketDetail, localizer: this.localizer, existingTicketDetail: ticketDetail);
                    }
                ...
                case Constants.UpdateExpertListAction:
                    ...
                    await this.cardHelper.UpdateManageExpertsCardInTeamAsync(turnContext, onCallExpertsDetail, this.onCallSupportDetailSearchService, this.onCallSupportDetailStorageProvider, this.localizer);
                    await this.activityHelper.SendMentionActivityAsync(onCallExpertsDetail.OnCallExperts, turnContext: turnContext, logger: this.logger, localizer: this.localizer, memoryCache: this.memoryCache, cancellationToken: cancellationToken);
            ...
            CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                string searchQuery = this.searchHelper.GetSearchQueryString(messageExtensionQuery);
                ...
                {
                    onCallSMEUsers = await this.cardHelper.GetOnCallSMEUserListAsync(turnContext, this.onCallSupportDetailSearchService, this.teamId, this.memoryCache, this.logger);
                    return new MessagingExtensionResponse
                    {
                        ComposeExtension = await this.searchHelper.GetSearchResultAsync(searchQuery, messageExtensionQuery.CommandId, messageExtensionQuery.QueryOptions.Count, messageExtensionQuery.QueryOptions.Skip, this.ticketSearchService, this.localizer, turnContext.Activity.From.AadObjectId, onCallSMEUsers),
                    };
                }
                ...
                if (turnContext != null && teamsChannelData.Team != null && teamsChannelData.Team.Id == this.teamId && (messageExtensionQuery.CommandId == Constants.UrgentCommandId || messageExtensionQuery.CommandId == Constants.AssignedCommandId || messageExtensionQuery.CommandId == Constants.UnassignedCommandId))
                {
                    return new MessagingExtensionResponse
                    {
                        ComposeExtension = await this.searchHelper.GetSearchResultAsync(searchQuery, messageExtensionQuery.CommandId, messageExtensionQuery.QueryOptions.Count, messageExtensionQuery.QueryOptions.Skip, this.ticketSearchService, this.localizer),
                    };
                }
